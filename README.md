...
In general, dynamic programming is used to solve the optimization problems. It is done from the bottom-up with tabulation. After the subproblems are solved, their solutions are used to more easily solve the larger problems. For example, the solution for optimizing a problem where there are 2 slots per row can use the solution from a problem where the is 1 slot per row.
